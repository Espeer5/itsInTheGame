...
def plot_predictions(game, top_k, tau, lamb, alpha_pch, alpha_l1):
    ...
    plt.plot(pch_p, pch_q, 'ro', label='PCH, t={:.3f}, a={:.3f}'.format(tau, alpha_pch))
    plt.plot(qre_p, qre_q, 'bo', label='QRE , l={:.3f}'.format(lamb))
    plt.plot(ne_p, ne_q, 'go', label='Nash Equilibrium')
    plt.plot(l1_p, l1_q, 'ko', label='Level 1, a={:.3f}'.format(alpha_l1))
    plt.xlim((-0.1, 1.4))
    plt.ylim((-0.1, 1.4))
    plt.xlabel('p')
    plt.ylabel('q')
